...
parser.add_argument(
    "-u",
    help="Target URL. If not provided, target directory will be scanned for har files.",
    nargs='?'
)
parser.add_argument(
    "-d",
    help="Target directory (default is \"hars\"). If URL is provided, directory will store har files. "
         "If URL is not provided, directory will be scanned. ",
    nargs='?'
)
parser.add_argument(
    "-s",
...
parser.add_argument(
    "-c",
    help="File containing JSON formatted cookies to set in driver (with target URL only)",
    nargs='?'
)
parser.add_argument(
    "-i",
...
parser.add_argument(
    '--p',
    help="Flag, remove unnecessary parameters (may dramatically increase run time)",
    action='store_true'
)
